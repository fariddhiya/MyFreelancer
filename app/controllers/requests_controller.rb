class RequestsController < ApplicationController
  ...
  def create
    ...
    if @request.save 
      redirect_to requests_path, notice: "save..."
    else 
  ...
  def update
    if @request.update(request_params)
      redirect_to requests_path, notice: "update..."
    else
      redirect_to request.referrer, flash: {error: @request.errors.full_messages.join(", ")}
    end
  end
  ...
  def destroy
    @request.destroy
    redirect_to request_path, notice: "Request deleted"
  end
  ...
  def request_params
    params.require(:request).permit(:title, :description, :delivery, :category_id, :attachment_file, :budget)
  end
end
